...
namespace Levels
{
	...
	Level1::Level1(Map map_, Space* HUDSpace) : Level("Level1", HUDSpace),
		map(map_),
	...
	void Level1::Initialize()
	{
		...
		//---------------Load HUD Level------------------------
		Space* HUDSpace = GetOtherSpace();

		// Set initial level to the HUD level - HUDSpace.
		if (HUDSpace != nullptr)
			HUDSpace->SetLevel(new Levels::HUDLevel(GetSpace()));
		//-------------------------------------------------------
	...
	void Level1::Update(float dt)
	{
		...
		if (playerCount == 1)
		{
			...
			{
				GetSpace()->SetLevel(new Levels::LevelSelect(GetOtherSpace()));
			}
		}
	...
	void Level1::Shutdown()
	{
		...
		// Set initial level to the HUD level - HUDSpace.
		if (HUDSpace != nullptr)
			HUDSpace->SetLevel(new Levels::HUDEmpty(GetSpace()));
	}
